...
export const IDado = ({ ruta }) => {
  ...
  const buscarJuego = async (idjuegolocal) => {
    try {
      const response = await axiosBase.get(
        "/juegos/buscarJuego/" + idjuegolocal
      );
      setdataPregunta([
        {
          cara: 1,
  ...
  const respuestaDado = async (pregunta, respuesta, id) => {
    ...
    try {
      const response = await axiosBase.post("/dado/respuestaDado", {
        pregunta: pregunta,
        respuesta: respuesta,
        id: id, // tabla juegos
      });
  ...
  const funcionesdado = () => {
    ...
    function rollDice() {
      if (shoot == 0) {
        ...
        const registrosFiltrados = dataResultados.filter(
          (registro) => registro.resultado === valorAleatorio
        );
        ...
        if (registrosFiltrados.length > 0) {
          ...
          const registroSeleccionado =
            registrosFiltrados[
              Math.floor(Math.random() * registrosFiltrados.length)
            ];
          ...
          dice.style.transform =
            "rotateX(" + primerValor + "deg) rotateY(" + segundoValor + "deg)";
    ...
    function showResult(resultadoDado) {
      ...
      setTimeout(() => {
        ...
        const pregunta = dataPregunta.filter(
          (registro) => registro.cara == resultadoDado
        );
  ...
  useEffect(() => {}, []); // El segundo argumento es un arreglo vacío para que se ejecute solo una vez cuando se monta el componente
          ...
          {!loading && (
            <>
              <div className="h-full w-2/5 mx-3 px-6 bg-gray-200 rounded-xl border-4 overflow-hidden shadow-lg grid items-center">
                <p
                  className="text-gray mt-5 w-full text-[28px]"
                  id="question_123"
                >
                  Pregunta
                </p>
              ...
              <div className="h-full w-2/5 mx-3 px-6 bg-red-400 rounded-xl border-4 overflow-hidden shadow-lg grid items-center justify-center">
                <div className="container">
                  <div
                    className="dice flex justify-center align-center"
                    id="dice"
                  >
